...
const MIRROR_KEY = KEY("mirror");        // 2차 안전: 마지막 상태를 이 기기에 거울 저장
const MIRROR_AT_KEY = KEY("mirror-at");  // 거울 저장 시각(ISO)
const EXT_BACKUP_AT_KEY = KEY("ext-backup-at");  // 마지막 외부(GitHub) 백업 시각(ISO)
...
const C = {
  ...
  navy:BRAND.navy,
  orange:BRAND.accent,  orangeL:BRAND.accentLight, orangeD:BRAND.accentDark,
...
const fmt=(n,u)=>{
  ...
  // 금액(원)은 억/만 단위로 줄이지 않고 0을 전부 붙여 전체 자릿수로 표시 (예: 300,000,000원)
  if(u==="원") return Math.round(numF(n)).toLocaleString()+"원";
...
// 삭제 확인 — '삭제'를 직접 입력해야 삭제 가능(실수 방지). 휴지통 경유라 복구 가능.
const ConfirmDelete=({open,title,desc,word="삭제",onOk,onCancel})=>{
  const [val,setVal]=useState("");
  useEffect(()=>{ if(open)setVal(""); },[open]);
  if(!open) return null;
  const ok=val.trim()===word;
  return(
    <div onClick={onCancel} style={{position:"fixed",inset:0,backgroundColor:"rgba(0,0,0,0.55)",zIndex:2000,display:"flex",alignItems:"center",justifyContent:"center",padding:"0 24px"}}>
      <div onClick={e=>e.stopPropagation()} style={{backgroundColor:"#FFFFFF",borderRadius:20,padding:"24px 22px",width:"100%",maxWidth:340,boxShadow:"0 8px 40px rgba(0,0,0,0.2)"}}>
        <p style={{margin:"0 0 6px",fontSize:16,fontWeight:900,color:"#0F1F5C"}}>{title}</p>
...
const downloadStateBackup=(D)=>{
  ...
  const blob=new Blob([JSON.stringify({_app:BRAND.backupTag,_backupAt:new Date().toISOString(),...shared},null,2)],{type:"application/json"});
  const url=URL.createObjectURL(blob);
  const a=document.createElement("a");a.href=url;a.download=`${BRAND.backupTag}-backup_${new Date().toISOString().slice(0,19).replace(/[:T]/g,"-")}.json`;a.click();URL.revokeObjectURL(url);
};
...
export default function App(){
  ...
  const [viewMode,setViewMode]=useState(()=>localStorage.getItem(KEY("view"))||((typeof window!=="undefined"&&window.innerWidth>=1024)?"pc":"mobile"));
  useEffect(()=>{ localStorage.setItem(KEY("view"),viewMode); },[viewMode]);
  ...
  const pushExternalBackup=async(reason)=>{
    try{
      const shared=pickShared(D);
      const content=JSON.stringify({_app:BRAND.backupTag,_backupAt:new Date().toISOString(),_reason:reason||"manual",...shared},null,2);
  ...
  if(!loaded) return(
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:"100vh",gap:14,fontFamily:"'Pretendard',sans-serif",color:"#9CA3AF"}}>
      <div style={{width:36,height:36,borderRadius:11,background:`linear-gradient(135deg,${BRAND.accent},${BRAND.accentDark})`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,color:"#fff",fontWeight:900}}>{BRAND.logoLetter}</div>
    ...
    <div style={{display:"flex",height:"100vh",backgroundColor:"#F9FAFB",fontFamily:"'Pretendard','Apple SD Gothic Neo',sans-serif",overflow:"hidden",width:"100%"}}>
      <aside style={{width:216,backgroundColor:"#FFFFFF",borderRight:"1px solid #F2F4F6",display:"flex",flexDirection:"column",flexShrink:0}}>
        <div style={{padding:"16px 16px 13px",display:"flex",alignItems:"center",gap:9,borderBottom:"1px solid #F4F4F5"}}>
          <div style={{width:30,height:30,borderRadius:9,background:`linear-gradient(135deg,${BRAND.accent},${BRAND.accentDark})`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,color:"#fff",fontWeight:900}}>{BRAND.logoLetter}</div>
          <div><p style={{margin:0,fontSize:14.5,fontWeight:900,color:"#0F1F5C",lineHeight:1.1}}>{BRAND.appName}</p><p style={{margin:0,fontSize:9.5,color:BRAND.accent,fontWeight:800}}>{BRAND.appSubtitle}</p></div>
        </div>
      ...
      <div style={{backgroundColor:"#FFFFFF",borderBottom:"1px solid #F2F4F6",padding:"12px 18px 10px",flexShrink:0}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            <div style={{width:28,height:28,borderRadius:8,background:`linear-gradient(135deg,${BRAND.accent},${BRAND.accentDark})`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,color:"white",fontWeight:900,letterSpacing:-1}}>{BRAND.logoLetter}</div>
            <div>
              <h1 style={{margin:0,fontSize:15,fontWeight:900,color:"#0F1F5C",lineHeight:1}}>{pi?.icon} {pi?.label}</h1>
              <p style={{margin:0,fontSize:10,color:BRAND.accent,fontWeight:700}}>{BRAND.orgLabel}</p>
            </div>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            {viewToggle}
...
function AIPage({D,cu,add,rm}){
  ...
  const run=async()=>{
    ...
    try{
      const res=await fetch("/api/coach",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:BRAND.coachModel,max_tokens:1000,messages:[{role:"user",content:prompt}]})});
  ...
  const saveResult=()=>{
    ...
    add("aiReviews",{id:"ai"+Date.now(),userId:cu.id,type,question:type==="custom"?q:"",result,model:BRAND.coachModel,savedAt:new Date().toISOString(),label:type==="kpi"?"KPI 분석":type==="ab"?"메인KPI 비교":type==="weekly"?"주간점검 "+weekLabel(weekKey()):"질문: "+q.slice(0,30)});
    setSaved(true);
  };
