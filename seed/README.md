...
mkt-links/{id}      // 추적링크: { url, label, product, channel, contentType, campaign, utm*, clicks(자동), ... }
mkt-perf/{id}       // 링크별 실측(수기): { impressions, orders, revenue, spend }  (id=링크 id)
promotions/{id}     // 프로모션 성과: { name, startDate, endDate, products, baselineRevenue, actualRevenue, spend }
```

### 마케팅 성과 (추적링크 자동 유입 집계)
- 공개 페이지 **`g.html`** (`/g?l=<id>`) 이 `mkt-links/{id}.clicks` 를 자동 +1 하고 목적지로 리다이렉트.
- 대시보드 [마케팅 개요]·[추적링크]·[프로모션 성과] 탭에서 링크 생성/실측 입력/ROAS·전환율·CPA·손익분기ROAS·증분매출 계산.
- 규칙: `mkt-links` read=공개(리다이렉트용, 재무값 없음)·clicks 1증가만 공개, 생성/편집은 허용목록.
  `mkt-perf`·`promotions` 는 허용목록 계정만.
